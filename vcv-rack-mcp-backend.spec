...
# PyInstaller spec — produce vcv-rack-mcp-backend.exe for Tauri NSIS embedding.
# Fleet standard: strip=False, upx=False, noarchive=True (see tauri_nsis_building.md).
# Usage (from repo root):
#   uv run pyinstaller vcv-rack-mcp-backend.spec --distpath dist --clean --noconfirm

block_cipher = None

a = Analysis(
    ["src/vcv_rack_mcp/__main__.py"],
    pathex=["src"],
    binaries=[],
    datas=[("src/vcv_rack_mcp", "vcv_rack_mcp")],
    hiddenimports=[
        "uvicorn.logging",
        "uvicorn.loops",
